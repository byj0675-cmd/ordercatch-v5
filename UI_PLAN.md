...
### Phase 3 — 바텀 시트 모달
- [ ] 기존 수기주문 모달 → 하단에서 슬라이드업 바텀 시트로 교체
- [ ] 딤 배경 클릭 시 닫기
- [ ] 모바일 풀스크린, 데스크탑 최대 높이 80vh
- [ ] 입력 완료 버튼 → 스낵바 Toast("주문이 등록되었습니다 ✅")

### Phase 4 — 플로팅 액션 버튼 (FAB)
- [ ] 우측 하단 고정 FAB: `＋ 주문 등록`
- [ ] FAB 호버 시 `🖨️ 프린트` 버튼 펼쳐짐 (스피드 다이얼)
- [ ] 스크롤해도 항상 화면에 고정

### Phase 5 — 프린트 뷰
- [ ] `🖨️ 오늘 주문 출력` 기능
- [ ] `@media print` CSS: 버튼/헤더 숨기고 테이블만 출력
- [ ] 출력 레이아웃: 번호 / 고객명 / 상품 / 픽업시간 / 레터링 문구 / 상태
- [ ] A4 최적화
