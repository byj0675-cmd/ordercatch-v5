...
export default function SettingsModal({ store, onClose }: SettingsModalProps) {
  const [activeTab, setActiveTab] = useState<"general" | "webhook" | "link">("general");
  const { profile, updateStoreProfile } = useStoreProvider();
  
  const [isEditing, setIsEditing] = useState(false);
  const [editName, setEditName] = useState(profile?.store_name || store.name);
  const [editCategory, setEditCategory] = useState(profile?.category || store.type);
  ...
  const finalStoreSlug = profile?.store_slug || store.id;
  const finalStoreName = profile?.store_name || store.name;
    ...
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
        ...
        <div style={{ padding: "20px 28px 0", display: "flex", gap: 4 }}>
          {(["general", "webhook", "link"] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === "general" ? "⚙️ 일반" : tab === "webhook" ? "🔗 웹훅" : "🛒 주문 링크"}
            </button>
          ))}
        </div>
